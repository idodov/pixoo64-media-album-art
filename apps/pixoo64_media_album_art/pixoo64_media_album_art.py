...
class Pixoo64_Media_Album_Art(hass.Hass):
    ...
    def pixoo_run(self, media_state):
        ...
        if media_state in ["playing", "on"]:  # Check for playing state
            ...
            if title != "TV" and title is not None:
                ...
                textid = 0
                text_string = self.convert_text(f"{artist} - {title}") if artist else self.convert_text(title)
                ...
                if self.SHOW_TEXT and not self.FALLBACK:
                    ...
                    text_temp = { "TextId":textid, "type":22, "x":0, "y":48, "dir":dir, "font":2, "TextWidth":64, "Textheight":16, "speed":100, "align":2, "TextString": text_string, "color":recommended_font_color }
                    moreinfo["ItemList"].append(text_temp)

                if self.SHOW_CLOCK and not self.FALLBACK:
                    textid +=1
                    x = 44 if self.LOCACTION_CLOCK == "Right" else 3
                    clock_item =  { "TextId":textid, "type":5, "x":x, "y":3, "dir":0, "font":18, "TextWidth":32, "Textheight":16, "speed":100, "align":1, "color":recommended_font_color }
    ...
    def process_picture(self, picture):
        ...
        try:
            ...
            bright_opposite_color = tuple(min(255, c + brightness_factor) for c in opposite_color)
            recommended_font_color = '#%02x%02x%02x' % bright_opposite_color
            ...
            if self.SHOW_CLOCK:
                lpc = (43, 2, 62, 9) if self.LOCACTION_CLOCK == "Right" else (2, 2, 21, 9)
                lower_part_img = img.crop(lpc)
                enhancer_lp = ImageEnhance.Brightness(lower_part_img)
                lower_part_img = enhancer_lp.enhance(0.3)
                img.paste(lower_part_img, lpc)
    ...
    def has_bidi(self, text):
        ...
        bidi_regex = f"[{hebrew}|{arabic}|{syriac}|{thaana}|{nkoo}|{rumi}|{arabic_math}|{symbols}|{old_persian_phaistos}|{samaritan}|{bidi_marks}]"
