...
try:
    ...
    print("The 'requests' module is not installed or not available. It's mandatory!")

try:
    import numpy as np
except ImportError:
    print("The 'numpy' module is not installed or not available. Crop feature won't work")

try:
    from unidecode import unidecode
    unicode_available = True
except ImportError:
    print("The 'unidecode' module is not installed or not available. Special chars might not display")
    unicode_available = False

try:
    from bidi import get_display
except ImportError:
    print("The 'bidi' module is not installed or not available. RTL texts will display reversed")
...
# Unicode ranges for RTL languages and symbols
hebrew = r"\u0590-\u05FF"
...
class Pixoo64_Media_Album_Art(hass.Hass):
    ...
    def load_config(self) -> None:
        ha_args = self.args.get('home_assistant', {})
        self.MEDIA_PLAYER = ha_args.get("media_player", "media_player.era300")
        self.TOGGLE = ha_args.get("toggle", "input_boolean.pixoo64_album_art")
        self.HA_URL = ha_args.get("ha_url", "http://homeassistant.local:8123")
        self.SENSOR = ha_args.get("pixoo_sensor", "sensor.pixoo64_media_data")
        self.LIGHT = ha_args.get("light", None)
        ...
        self.CLOCK_ALIGN = pixoo_args.get("clock_align", "Left")
        self.TV_ICON = pixoo_args.get("tv_icon", True)

        text_args = pixoo_args.get('text', {})
        self.SHOW_TEXT = text_args.get("enabled", False)
        self.CLEAN_TITLE = text_args.get("clean_title", True)
        self.FONT = text_args.get("font", 2)
        self.FONT_COLOR = text_args.get("color", True)
        self.TEXT_BG = text_args.get("background", True)

        crop_args = pixoo_args.get('crop', {})
        self.CROP_BORDERS = crop_args.get("enabled", True)
        self.CROP_EXTRA = crop_args.get("extra", True)

    def update_attributes(self, entity: str, attribute: str, old: str, new: str, kwargs: Dict[str, Any]) -> None:
        try:
    ...
    def clean_title(self, title: str) -> str:
        patterns = [
            r"\.(mp3|wav|flac|ogg|aac|m4a)$",
            r"\((?:.*?Remaster(?:ed)?|.*?Version).*?\)|\[(?:.*?Remaster(?:ed)?|.*?Version).*?\]",
            r"\((?:.*?Mix(?:ed)?).*?\)|\[(?:.*?Remix(?:ed)?).*?\]",
            r"-(?:.*?Remaster(?:ed)?).*?$",
            r"-(?:.*?Radio Edit(?:ed)?).*?$",
            r"^\d+\s*-\s*"
    ...
    def pixoo_run(self, media_state: str) -> None:
        ...
        if media_state in ["playing", "on"]:
            ...
            title = self.clean_title(title) if self.CLEAN_TITLE else title
            ...
            if title != "TV" and title is not None:
                artist = self.get_state(self.MEDIA_PLAYER, attribute="media_artist") or ""
                normalized_title = unidecode(title) if unicode_available else title
                normalized_artist = unidecode(artist) if artist and unicode_available else artist
                ...
                payload = {
                    ...
                    "CommandList": [
                        ...
                        {"Command": "Draw/SendHttpGif", "PicNum": 1, "PicWidth": 64, "PicOffset": 0, "PicID": 0, "PicSpeed": 1000, "PicData": gif_base64}
                    ]
                }

                moreinfo = {"Command": "Draw/SendHttpItemList", "ItemList": []}
                textid = 0
                text_track = f"{artist} - {title}" if artist else title
                if len(text_track) > 14:
                    text_track += "       "

                text_string = self.convert_text(text_track)
                dir = 1 if self.has_bidi(text_string) else 0

                if self.send_pic:
                    self.album_name = album_name_check
                ...
                if self.TV_ICON:
                    payload = {
                        ...
                        "CommandList": [
                            {"Command": "Channel/OnOffScreen", "OnOff": 1},
                            {"Command": "Draw/ResetHttpGifId"},
                            {"Command": "Draw/SendHttpGif", "PicNum": 1, "PicWidth": 64, "PicOffset": 0, "PicID": 0, "PicSpeed": 1000, "PicData": zlib.decompress(TV_ICON).decode()}
                        ]
                    }
                else:
                    payload = {
    ...
    def process_picture(self, picture: str) -> Tuple[str, str, str, int, int, str, Tuple[int, int, int], Tuple[int, int, int], Tuple[int, int, int], str]:
        ...
        try:
            ...
            self.log(f"Error processing image. Using default values: {e}")
            self.FALLBACK = True
    ...
    def get_image(self, picture: str) -> Optional[Image.Image]:
        try:
    ...
    def crop_borders(self, img: Image.Image) -> Image.Image:
        ...
        try:
            ...
            img = temp_img.crop((y_min, x_min, y_max, x_max))
    ...
    def send_pixoo(self, payload_command: Dict[str, Any]) -> None:
        response = requests.post(self.URL, headers=HEADERS, json=payload_command)
    ...
    def get_pixels(self, img: Image.Image) -> List[int]:
        if img.mode == "RGB":
    ...
    def split_string(self, text: str, length: int) -> List[str]:
        words = text.split(' ')
        lines = []
        current_line = ''
    ...
    def create_payloads(self, normalized_artist: str, normalized_title: str, x: int) -> List[Dict[str, Any]]:
    ...
    def control_light(self, action: str, background_color_rgb: Optional[Tuple[int, int, int]] = None) -> None:
        service_data = {'entity_id': self.LIGHT}
        if action == 'on' and background_color_rgb:
    ...
    def most_vibrant_color(self, img: Image.Image) -> Tuple[int, int, int]:
        img.thumbnail((64, 64), Image.Resampling.LANCZOS)
    ...
    def has_bidi(self, text: str) -> bool:
