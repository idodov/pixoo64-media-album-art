"""
PIXOO64 Media Album Art - This feature automatically displays the album cover art of the track currently playing on the Divoom Pixoo64 screen
"""
import re
import base64
import requests
    ...
    def update_attributes(self, entity, attribute, old, new, kwargs):
        ...
        if input_boolean == "on":
            ...
            if media_state in ["playing", "on"]:  # Check for playing state
                new = self.get_state(self.MEDIA_PLAYER, attribute="media_title")
                if new != "TV" and new is not None:
                    ...
                    payload = {"Command":"Draw/CommandList", "CommandList":[
                        ...
                        {"Command": "Draw/SendHttpGif",
                            ...
                            "PicID": 0,
                            "PicSpeed": 1000,
                            "PicData": zlib.decompress(TV_ICON).decode() }]}
                    self.send_pixoo(payload)
            else:
